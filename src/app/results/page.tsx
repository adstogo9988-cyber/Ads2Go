...
missing: (reportCategories as any[]).reduce((acc: number, cat: any) => acc + cat.checks.filter((i: any) => i.status === "not_scanned").length, 0)
    ...
    const generatePDF = async () => {
        try {
            ...
            doc.text("Generated by ad2vo.com", 14, 30);

            doc.setFontSize(12);
            doc.setTextColor(0, 0, 0);
            doc.text(`Scanned URL: ${analysisUrl}`, 14, 42);
            doc.text(`Date: ${new Date().toLocaleDateString()}`, 14, 48);

            // 2. Overview Metrics
            doc.setLineWidth(0.5);
            doc.line(14, 54, pageWidth - 14, 54); // Horizontal line

            doc.setFont("helvetica", "bold");
            doc.text("Executive Summary", 14, 64);
            
            doc.setFont("helvetica", "normal");
            doc.text(`Overall Readiness Score: ${getOverallScore()}/100`, 14, 72);
            doc.text(`Critical Issues: ${issues.critical}`, 14, 78);
            doc.text(`Warnings: ${issues.warnings}`, 14, 84);
            doc.text(`Passed Checks: ${issues.passed}`, 14, 90);

            let finalY = 100;

            // 3. Category Data Tables
            const categories = reportCategories as any[];
                ...
                autoTable(doc, {
                    ...
                    didParseCell: function(data) {
                        if (data.section === 'body' && data.column.index === 0) {
                            ...
                            else if (status === 'FAIL') data.cell.styles.textColor = [220, 53, 69];
                            else if (status === 'WARN') data.cell.styles.textColor = [255, 193, 7];
                        }
                    }
                });
            ...
            doc.save(`${analysisUrl.replace(/[^a-z0-9]/gi, '_').toLowerCase()}_adsense_report.pdf`);
        } catch (err: any) {
