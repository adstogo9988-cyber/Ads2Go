        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="glass-panel bg-white rounded-[32px] p-8 md:p-10 border border-slate-100 flex flex-col hover:shadow-lg transition-all duration-300">
            ...
            <div className="pt-6 border-t border-slate-50 flex items-center justify-between mt-auto">
               ...
               <a href="#" className="text-slate-400 hover:text-slate-900 transition-colors flex items-center gap-2 text-xs font-semibold cursor-pointer">
          ...
          {/* Case Study 2 */}
          <div className="glass-panel bg-white rounded-[32px] p-8 md:p-10 border border-slate-100 flex flex-col hover:shadow-lg transition-all duration-300 relative overflow-hidden group">
            <div className="absolute top-0 right-0 p-8 opacity-[0.02] group-hover:opacity-[0.04] transition-opacity">
               <CheckCircle2 size={300} className="text-slate-900" />
            </div>
            
            <div className="flex justify-between items-start mb-8 relative z-10">
               <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center">
                     <span className="text-slate-600 font-black text-sm">TR</span>
                  </div>
                  <div>
                    <h4 className="text-sm font-bold text-slate-900">Tech Reviewer Pro</h4>
                    <span className="text-[10px] uppercase tracking-widest text-slate-400">Hardware Publisher</span>
                  </div>
               </div>
               <div className="px-3 py-1 bg-blue-50 text-blue-600 border border-blue-100 rounded-full text-[10px] font-bold uppercase tracking-widest">
                  Scaled
               </div>
            </div>

            <div className="mb-6 pb-6 border-b border-slate-50 relative z-10">
               <div className="flex items-start gap-3 mb-3">
                  <AlertTriangle className="text-red-400 shrink-0 mt-0.5" size={16} />
                  <div>
                     <span className="text-xs font-bold text-slate-800 block mb-1">Previous Status: Site Behavior Ban</span>
                     <p className="text-xs text-slate-500 font-light leading-relaxed">Account was flagged for "Site Behavior: Navigation". The user exhausted all appeals and was bleeding traffic value.</p>
                  </div>
               </div>
            </div>
            ...
            <div className="pt-6 border-t border-slate-50 flex items-center justify-between mt-auto relative z-10">
               <div className="flex flex-col">
                  <span className="text-2xl font-light text-slate-900">$14.2k</span>
                  <span className="text-[9px] uppercase tracking-widest text-slate-400">Monthly Yield Reclaimed</span>
               </div>
               <a href="#" className="text-slate-400 hover:text-slate-900 transition-colors flex items-center gap-2 text-xs font-semibold cursor-pointer">
                 View Case <ArrowRight size={14} />
